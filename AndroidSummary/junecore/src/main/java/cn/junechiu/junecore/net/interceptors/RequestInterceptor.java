...
import cn.junechiu.junecore.utils.CharacterHandler;
import cn.junechiu.junecore.utils.ZipHelper;
...
public class RequestInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        boolean logRequest = printLevel == Level.ALL ||
                (printLevel != Level.NONE && printLevel == Level.REQUEST);
        if (logRequest) {
            boolean hasRequestBody = request.body() != null;
            //打印请求信息
            ALogger.d("Request_Info", "Params : 「 %s 」%nConnection : 「 %s 」%nHeaders : %n「 %s 」",
                    hasRequestBody ? parseParams(request.newBuilder().build().body()) : "Null",
                    chain.connection(), request.headers());
        }

        boolean logResponse = printLevel == Level.ALL ||
                (printLevel != Level.NONE && printLevel == Level.RESPONSE);
        long t1 = logResponse ? System.nanoTime() : 0;
        Response originalResponse;
        try {
            originalResponse = chain.proceed(request);
        } catch (Exception e) {
            ALogger.d("Request_Info Error: " + e);
            throw e;
        }
        ...
        if (logResponse) {
            ...
            //打印响应时间以及响应头
            ALogger.d("Response_Info", "Received response in [ %d-ms ] , [ %s ]%n%s",
